...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
	...
	particleRootParamenters[2].DescriptorTable.pDescriptorRanges = descriptorRangeForInstancing;
	particleRootParamenters[2].DescriptorTable.NumDescriptorRanges = _countof(descriptorRangeForInstancing);
...
#pragma region スフィア
	...
	//06_02
	//ModelDataを使う
	ModelData modelData;//UVチェッカー = LoadObjFile("Resource", "plane.obj")
	//Primitive
...
#pragma endregion
	...
	//D3D12_CPU_DESCRIPTOR_HANDLE textureSrvHandleCPU = GetCPUDescriptorHandle(srvDscriptorHeap, desriptorSizeSRV, 3);
	//D3D12_GPU_DESCRIPTOR_HANDLE textureSrvHandleGPU = GetGPUDescriptorHandle(srvDscriptorHeap, desriptorSizeSRV, 3);
	...
	D3D12_CPU_DESCRIPTOR_HANDLE instancingSrvHandleCPU = GetCPUDescriptorHandle(srvDscriptorHeap, desriptorSizeSRV, 3);
	...
	while (msg.message != WM_QUIT)
	{
		...
		else
		{
			...
			Matrix4x4 particleViewProjectionMatrix = MatrixMath::Multiply(viewMatrix, projectionMatrix);
			//パーティクル用
			for (uint32_t index = 0; index < kNumInstance; ++index)
			{
				...
				Matrix4x4 worldViewProjectionMatrix = MatrixMath::Multiply(worldMatrix, particleViewProjectionMatrix);
